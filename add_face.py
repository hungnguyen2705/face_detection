...
print("--- TOOL THU THẬP DỮ LIỆU ĐA GÓC ---")
name = input("Nhập tên người muốn thêm (VD: Huy): ")
...
while True:
    ...
    if current_phase_idx >= len(PHASES):
        cv2.putText(display_frame, "DA HOAN THANH! NHAN 'Q' DE THOAT", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    ...
    if not is_capturing:
        ...
        if photos_taken >= IMAGES_PER_BATCH:
            ...
            photos_taken = 0
            current_phase_idx += 1 
            print(f"--- Xong bước {phase_name}. Chuyển bước tiếp theo ---")
